...
describe('Dashboard', () => {
  it('should be able to list all the products from your api', async () => {
    ...
    await wait(() => expect(getByText('Super Mario')).toBeTruthy(), {
      timeout: 200,
    });

    expect(getByText('Super Mario')).toBeTruthy();
    expect(getByText('Super Mario Odyssey')).toBeTruthy();
    expect(getByTestId('add-game-312')).toBeTruthy();

    expect(getByText('warfare')).toBeTruthy();
    expect(getByText('Call Of Duty Infinite Warfare')).toBeTruthy();
    expect(getByTestId('add-game-201')).toBeTruthy();
  });
});
